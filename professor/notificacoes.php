...
?>  
          <!-- Page Heading -->
          <h1 class="h3 mb-1 text-gray-800">Notificar Alunos</h1>
          <p class="mb-4">Neste espaço você pode notificar seu aluno ou responsável pelo mesmo sobre algum trabalho, sugestão ou  algo mais que deseje comentar.</p>
            <form name="form_disc" method="post" action="">
          <div class="form-group">
            <label for="exampleFormControlTextarea1">Escolha o aluno:</label>
            <select class="form-control" name="alunoEscolhido">
            <option>Selecionar Aluno</option>
   <?php
    $resultI = mysqli_query($connect,"SELECT * FROM estudantes WHERE serie = '$curso'");
   ...
   ?>                
            ...
            <br>
            <button type="submit" class="btn btn-success mb-2" name="submeterNotificacao">Enviar Notificação</button> 
          </div>          
            </form> 
            
            <form name="form_disc" method="post" action="">
          <div class="form-group">
            <label for="exampleFormControlTextarea1">Notificar toda a turma:</label>
            <select class="form-control" name="turmaEscolhida">
            ...
            <button type="submit" class="btn btn-success mb-2" name="submeterNotificacaoTurma">Enviar Notificação</button> 
          </div>          
            </form>             
...
<?php
    if (isset($_POST['submeterNotificacaoTurma'])){
        $turma_escolhda = $_POST['turmaEscolhida'];
        $mensagem = $_POST['mensagemEnviar2'];
        $date = date("d/m/Y H:i:s");

      $resultI = mysqli_query($connect,"SELECT code FROM estudantes WHERE serie = '$turma_escolhda'");
        if (mysqli_num_rows($resultI)==''){
            $aluno_turma = '';
    }else{
        while($resultados = mysqli_fetch_array($resultI)){
            $aluno_turma = $resultados['code'];
               
        
        $sql_4 = mysqli_query($connect,"INSERT INTO central_mensagem (date, emissor, receptor, mensagem, status) VALUES ('$date', '$code', '$aluno_turma', '$mensagem', 'Aguarda resposta')");

...
<?php $code = $_GET['code']; $date = date("d/m/Y H:i:s"); ?>
...
<?php }} ?>
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Cadastre abaixo as informações do seu fórum!</h1>
...
$ext = strtolower(substr($_FILES['trabalho']['name'],-5));
if ($ext==""){
  $trabalho = 'Sem Imagem.png';
}else{
  @$trabalho = date("Y.m.d-H.i.s") .$code.$ext; 
}   
...
$sql_3 = mysqli_query($connect,"SELECT * FROM disciplinas WHERE disciplina = '$dis'");
	while($res_3 = mysqli_fetch_array($sql_3)){
		$curso = $res_3['curso'];
        
$sql_4 = mysqli_query($connect,"INSERT INTO foruns (date, status, professor, curso, disciplina, tema, detalhes, data_entrega,anexo) VALUES ('$date', 'Ativo', '$code', '$curso', '$dis', '$tema', '$detalhes','$encerramento','$trabalho')");

(move_uploaded_file($_FILES['trabalho']['tmp_name'], "../foruns/".$trabalho));
        
echo "<script language='javascript'>window.alert('Fórum cadastrado com sucesso!');</script>";
...
<table class="table table-bordered">
  <tr>
    <td><strong>N&ordm; trabalho</strong></td>
  ...
  <tr>
    ...
    <?php
    $sql_1 = mysqli_query($connect,"SELECT * FROM trabalhos_extras ORDER BY id DESC LIMIT 1");
	if(mysqli_num_rows($sql_1) == ''){
		echo "1";
	}else{
		while($res_1 = mysqli_fetch_array($sql_1)){
    ...
    <td><input class="form-control" disabled type="text" value="<?php echo date("d/m/Y H:i:s"); ?>"></td>
    <td><input class="form-control" name ="dis" disabled type="text" value="<?php echo $dis; ?>">
   </td>
  </tr>
  <tr>
    <td width="216"><strong>Data de entrega</strong></td>
    <td colspan="2"><strong>Tema</strong></td>
  </tr>
  <tr>
    <td><input class="form-control" type="text" name="encerramento" value=""></td>
    <td colspan="3"><input class="form-control" type="text" name="tema" value=""></td>
  </tr>
  <tr>
    <td colspan="3"><strong>Descrição do fórum:</strong></td>
  </tr>
  <tr>
    <td colspan="3">
  ...
  <tr>
    <td><strong>Anexar Imagem:</strong></td>
    <td colspan="2"><input type="file" name="trabalho" id="fileField"></td>
  </tr>    
  <tr>
    <td colspan="1"><input class="btn btn-success" type="submit" name="button" id="button" value="Cadastrar"></td>
      <td colspan="2"><a href="../professor/foruns.php?dis=<?php echo base64_encode($dis); ?>'"class="btn btn-warning"  id="button">Ver Fóruns</a></td>
  </tr>
  </table>
  </form>
</div>

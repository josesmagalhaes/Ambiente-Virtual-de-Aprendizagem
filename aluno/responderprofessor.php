...
<?php }} ?>
        ...
        <!-- End of Topbar -->
<?php
            $aluno_x = base64_decode($_GET['aluno_x']);
?>
        <!-- Begin Page Content -->
        <div class="container-fluid">
          <!-- Page Heading -->
          <h1 class="h3 mb-1 text-gray-800">Responder Professor</h1>
          <p class="mb-4">Neste espaço você pode responder seu professor pelo mesmo sobre algum trabalho, sugestão ou  algo mais que deseje comentar.</p>
            <form name="form_disc" method="post" action="">
          <div class="form-group">
            <label for="exampleFormControlTextarea1">Porfessor:</label>
            <select class="form-control" name="alunoEscolhido">
                     <?php 
                    $nome_aluno_x = mysqli_query($connect,"SELECT nome FROM acesso_ao_sistema WHERE CODE = '$aluno_x'");
                        if (mysqli_num_rows($nome_aluno_x)==''){
                        }else{
                            while($res_x = mysqli_fetch_array($nome_aluno_x)){
                                $alx = $res_x['nome'];
                     ?>                 
                
               <option><?php echo $alx; $codigoAluno = $aluno_x;?></option>
                <?php }}?>
            </select>
            ...
            <br>              
            <button type="submit" class="btn btn-success mb-2" name="submeterNotificacao">Responder</button> 
          </div>          
            </form> 
...
<?php
    if (isset($_POST['submeterNotificacao'])){
        ...
        $sqlCODE = mysqli_query($connect,"SELECT code FROM acesso_ao_sistema WHERE nome = '$alunoEscolhido'");

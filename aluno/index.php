...
<?php }} ?>
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-1 text-gray-800">Alunos</h1>
          <p class="mb-4">No espaço abaixo, você aluno poderá acessar arquivos, videoaulas e trabalhos postados pelos professores.</p>
              <div class="card mb-4 py-3 border-left-danger">
                <div class="card-body">
                <button class="btn btn-outline-secondary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Videoaula</button>
                <div class="dropdown-menu">
                     <?php 
                    $disciplinas_1 = mysqli_query($connect,"SELECT disciplina FROM disciplinas WHERE curso = '$turmaPegar'");
                        if (mysqli_num_rows($disciplinas_1)==''){
                        }else{
                            while($resultadosDis_1 = mysqli_fetch_array($disciplinas_1)){
                                $diss_1 = $resultadosDis_1['disciplina'];
                     ?>                     
                  <a class="dropdown-item" href="videoaulas.php?dis=<?php echo base64_encode($diss_1);?>"><?php echo $diss_1;?></a>
                <?php }}?>    
                </div>                    
                </div>
              </div>
              ...
              <div class="card mb-4 py-3 border-left-warning">
                <div class="card-body">
                <button class="btn btn-outline-secondary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Fóruns</button>
                <div class="dropdown-menu">
                     <?php 
                    $disciplinas_1 = mysqli_query($connect,"SELECT disciplina FROM disciplinas WHERE curso = '$turmaPegar'");
                        if (mysqli_num_rows($disciplinas_1)==''){
                            ...
                            while($resultadosDis_1 = mysqli_fetch_array($disciplinas_1)){
                                $diss_1 = $resultadosDis_1['disciplina'];
                     ?>                     
                  <a class="dropdown-item" href="foruns.php?dis=<?php echo base64_encode($diss_1);?>"><?php echo $diss_1;?></a>
                <?php }}?>    
                </div>                    
                </div>
              </div>             
              <div class="card mb-4 py-3 border-left-success">
                  <div class="text-xs font-weight-bold text-success text-uppercase mb-1">&nbsp;&nbsp;&nbsp;Mural de Arquivos</div>
              <div class="card-body">
<?php 
        $file = mysqli_query($connect,"SELECT * FROM arquivos_professor WHERE curso = '$turmaPegar'");
            if (mysqli_num_rows($file)==''){
            }else{
                while($resFile = mysqli_fetch_array($file)){
                    $data = $resFile['date'];
                    $disciplina = $resFile['disciplina'];
                    $curso = $resFile['curso'];
                    $arquivo = $resFile['arquivo'];
...
<style>
  ...
  margin: 0;
                ...
                <p class="remp"><?php echo "<strong>Data:</strong> ". $data." <strong>Disciplina: </strong>".
                $disciplina;?></p><a href="../arquivos_professores/<?php echo $arquivo;?>" target="_blank"><?php echo $arquivo;?></a>

...
<?php }} ?>
        ...
        <div class="container-fluid">
          <!-- Page Heading -->
          <h1 class="h3 mb-1 text-gray-800">Enviar Mensagens</h1>
          <p class="mb-4">Neste espaço você pode notificar algum professor ou servidor sobre algum trabalho, sugestão ou  algo mais que deseje comentar.</p>
            <form name="form_disc" method="post" action="">
          <div class="form-group">
            <label for="exampleFormControlTextarea1">Professores:</label>
   ...
   ?>                
    ...
    <script src="https://cdn.ckeditor.com/4.14.0/full/ckeditor.js"></script>
          ...
          <!-- Content Row -->
            <form name="form_disc" method="post" action="">
          <div class="form-group">
            <label for="exampleFormControlTextarea1">Servidores:</label>
            <select class="form-control" name="alunoEscolhido">
            <option>Selecionar Servidor</option>
   <?php
    $resultI = mysqli_query($connect,"SELECT * FROM funcionarios");
    if (mysqli_num_rows($resultI)==''){
    }else{
        while($resultados = mysqli_fetch_array($resultI)){
   ?>                
    ...
    <br>
            <button type="submit" class="btn btn-success mb-2" name="submeterNotificacaoServidor">Enviar Notificação</button> 
          </div>          
            </form>             
...
<?php
    if (isset($_POST['submeterNotificacaoServidor'])){
        $alunoEscolhido = $_POST['alunoEscolhido'];
        ...
        $sqlCODE = mysqli_query($connect,"SELECT code FROM funcionarios WHERE nome = '$alunoEscolhido'");
        if (mysqli_num_rows($sqlCODE)==''){
            $codeAlunoNot = "";
            }else{
            while($resultados = mysqli_fetch_array($sqlCODE)){

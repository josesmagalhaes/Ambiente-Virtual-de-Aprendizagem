...
<?php }} ?>
                  <a class="dropdown-item text-center small text-gray-500" href="../aluno/agenda.php">Ler todas as mensagens da Agenda</a>
              </div>
            </li>              
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-1 text-gray-800">Trabalhos a fazer</h1>
          <p class="mb-4">No espaço abaixo, você aluno poderá acompanhar os trabalhos lançados pelo seu professor.</p>
...
<?php
$sql_x = mysqli_query($connect,"SELECT serie FROM estudantes WHERE code = '$code'");
if(mysqli_num_rows($sql_x) == ''){
}else{
		while($res_x = mysqli_fetch_array($sql_x)){
        $serie = $res_x['serie'];
        }
}
  
$sql_1 = mysqli_query($connect,"SELECT * FROM trabalhos_extras WHERE status = 'Ativo' AND curso = '$serie'");
if(mysqli_num_rows($sql_1) == ''){
...
<div class="table-responsive-sm">
  ...
  <tr>
    <td><a class="btn btn-success" href="detalhes_do_trabalho.php?id=<?php echo $res_1['id']; ?>&pg=trabalho_extras" rel="superbox[iframe][515x400]">Mais detalhes</a></td>
    <?php 
	$sql_2 = mysqli_query($connect,"SELECT * FROM envio_de_trabalhos_extras WHERE id_trabalho = ".$res_1['id']." AND aluno = '$code'");
	if(mysqli_num_rows($sql_2) >= '1'){
?>
    <td colspan="2"><a class="btn btn-warning" href="enviar_trabalho.php?id=<?php echo $res_1['id']; ?>&pg=trabalho_extras&aluno=<?php echo $code; ?>&dis=<?php echo $res_1['disciplina']; ?>&entrega=<?php echo $res_1['data_entrega']; ?>" rel="superbox[iframe][400x190]">Você já enviou! Deseja enviar novamente?</a></td>      
<?php        
	}else{
	?>
    <td colspan="2"><a class="btn btn-success" href="enviar_trabalho.php?id=<?php echo $res_1['id']; ?>&pg=trabalho_extras&aluno=<?php echo $code; ?>&dis=<?php echo $res_1['disciplina']; ?>&entrega=<?php echo $res_1['data_entrega']; ?>" rel="superbox[iframe][400x190]">Enviar trabalho</a></td>
    <?php } ?>
    <td></td>
    <td>&nbsp;</td>
  </tr>

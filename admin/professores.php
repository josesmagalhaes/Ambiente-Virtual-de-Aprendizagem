...
<?php }} ?>
        ...
        <div class="container-fluid">
          ...
          <p class="mb-4">Abaixo todos os professores do ano letivo de 2020.</p>

          <!-- DataTales Example -->
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-success">Lista de Professores</h6>
            </div>
            <div class="card-body">
              <div class="table-responsive">
               <?php
                  $sql_1 = mysqli_query($connect,"SELECT * FROM professores WHERE nome != ''");
                  if(mysqli_num_rows($sql_1) == ''){
              ...
              ?> 
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                    <tr>
                      <td><h4>Código</td></h4>
                      <td><h4>Nome </td></h4>
                      <th><h4>Disciplinas</th></h4>
                      <th><h4>Remuneração</th></h4>
                      <th><h4>Status</th></h4>
                      <th><h4>Ações</th></h4>
                    </tr>
                    ...
                    <td><h5><?php echo mysqli_num_rows(mysqli_query($connect,"SELECT * FROM disciplinas WHERE professor = '$code'")); ?></h5></t5>
                    <td><h5>R$ <?php echo $res_1['salario']; ?></h5></td>
                    <td><h5><?php echo $res_1['status']; ?></h5></td>
                    <td>
                    <a class="a" href="professores.php?pg=todos&func=deleta&id=<?php echo $res_1['id']; ?>"><i title="Excluir Professor(a)" class="fas fa-trash-alt"></i></a>
                    <?php if($res_1['status'] == 'Inativo'){?>
                    <a class="a" href="professores.php?pg=todos&func=ativa&id=<?php echo $res_1['id']; ?>&code=<?php echo $res_1['code']; ?>"><i title="Ativar Professor(a)" class="fas fa-check-circle"></i></a>
                    <?php } ?>
                    <?php if($res_1['status'] == 'Ativo'){?>
                    <a class="a" href="professores.php?pg=todos&func=inativa&id=<?php echo $res_1['id']; ?>&code=<?php echo $res_1['code']; ?>"><i title="Inativar Professor(a)" class="fas fa-ban"></i></a>
                    <?php } ?>
                    <a class="a" href="professores.php?pg=todos&func=edita&id=<?php echo $res_1['id']; ?>"><i title="Editar Professor(a)" class="fas fa-edit"></i></a>

                    <a class="a" rel="superbox[iframe][930x500]" href="historico_professor.php?id=<?php echo $res_1['id']; ?>"><i title="Histórico deste Professor(a)" class="fas fa-eye"></i></a></td>
                  </tr>
                  <?php } ?> 
                  <?php }?>
<!-- Todo o bloco PHP ficará neste espaço-->
                  ...
                  mysqli_query($connect,"DELETE FROM professores WHERE id = '$id'");
                  ...
                  mysqli_query($connect,"UPDATE professores SET status = 'Ativo' WHERE id = '$id'");
                  ...
                  mysqli_query($connect,"UPDATE professores SET status = 'Inativo' WHERE id = '$id'");
                  ...
                  $sql_1 = mysqli_query($connect,"SELECT * FROM professores WHERE id = '$id'");
                    while($res_1 = mysqli_fetch_array($sql_1)){
                  ?>
                  ...
                  $sql_2 = mysqli_query($connect,"UPDATE professores SET nome = '$nome', cpf = '$cpf', nascimento = '$nascimento', formacao = '$formacao', graduacao = '$graduacao', pos_graduacao = '$pos_graduacao', mestrado = '$mestrado', doutorado = '$doutorado', salario = '$salario' WHERE id = '$id'");
                  if($sql_2 == ''){
                    echo "<script language='javascript'>window.alert('Ocorreu um erro tente novamente!');window.location='';</script>";
                  }else{
                    echo "<div class='alert alert-success' role='alert'>Professor alterado com sucesso!</div>";
                ...
                <a class="btn btn-success" href="/admin/professores.php" role="button">Home</a>
                <a class="btn btn-success" href="professores.php?pg=cadastra" role="button">Cadastrar novo professor</a>
                ...
                $sql_2 = mysqli_query($connect,"INSERT INTO professores (code, status, nome, cpf, nascimento, formacao, graduacao, pos_graduacao, mestrado, doutorado, salario) VALUES ('$code', 'Ativo', '$nome', '$cpf', '$nascimento', '$formacao', '$graduacao', '$pos_graduacao', '$mestrado', '$doutorado', '$salario')");
                if($sql_2 == ''){
                  echo "<script language='javascript'>window.alert('Ocorreu um erro ao cadastrar!');</script>";
                }else{
                  mysqli_query($connect,"INSERT INTO acesso_ao_sistema (status, code, senha, nome, painel) VALUES ('Ativo', '$code', '$cpf', '$nome', 'professor')");
                ...
                <form name="form1" method="post" action="">
                  <table class="table table-striped">
                    ...
                    <tr>
                      <td>
                      <?php
                      $sql_1 = mysqli_query($connect,"SELECT * FROM professores ORDER BY id DESC LIMIT 1");
                    if(mysqli_num_rows($sql_1) == ''){
                      $new_code = "87415978";
                    ?>

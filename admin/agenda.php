...
<?php }} ?>
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-1 text-gray-800">Notificar Turma</h1>
          <p class="mb-4">Neste espaço, você pode notificar todos os alunos de uma turma sobre algum trabalho, sugestão ou  algo mais que tenha vindo a acontecer.</p>
            <form name="form_disc" method="post" action="">
          <div class="form-group">
            <label for="exampleFormControlTextarea1">Escolha a turma:</label>
            <select class="form-control" name="turmaEscolhida">
            <option>Selecionar Turma</option>
   <?php
    $resultI = mysqli_query($connect,"SELECT * FROM cursos");
    if (mysqli_num_rows($resultI)==''){
    }else{
        while($resultados = mysqli_fetch_array($resultI)){
   ?>                
              <option><?php echo $resultados['curso'];?></option>
              <?php }}?>  
            </select>
            ...
            <button type="submit" class="btn btn-success mb-2" name="submeterNotificacao">Notificar Turma</button> 
          </div>          
            </form> 
            <a href="../admin/verificaragenda.php" class="btn btn-success" role="button">Verificar Envios</a> 
          <!-- Content Row -->
          <div class="row">

<?php
    if (isset($_POST['submeterNotificacao'])){
        $receptor = $_POST['turmaEscolhida'];
        $mensagem = $_POST['mensagemEnviar'];
        $date = date("d/m/Y H:i:s");


        $sql_4 = mysqli_query($connect,"INSERT INTO central_agenda (date, emissor, receptor, mensagem, status) VALUES ('$date', '$code', '$receptor', '$mensagem', 'Aguarda resposta')");
